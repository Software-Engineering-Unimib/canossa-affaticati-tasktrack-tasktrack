...
export default function Login() {
    ...
    // Pulisce i campi al caricamento
    useEffect(() => {
        setEmail('');
        setPassword('');
    }, []);
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            // 1. Login con Supabase (ora scriverà i cookie grazie alla modifica in lib/supabase.ts)
            ...
            // 2. CRUCIALE: Aggiorna la vista server per riconoscere il cookie appena settato
            router.refresh();
            ...
            setIsLoading(false); // Stop loading solo se c'è errore
        }
    };
                ...
                <div className="w-full max-w-md space-y-8">
                    ...
                    <form onSubmit={handleLogin} className="mt-8 space-y-6" autoComplete="off">
                        <div className="space-y-1">
                            <label htmlFor="email" className="block text-sm font-medium text-gray-700">Email Istituzionale</label>
                        ...
                        <div className="space-y-1">
                            <div className="flex items-center justify-between">
                                ...
                                <button type="button" onClick={() => setIsForgotOpen(true)} className="text-sm font-medium text-blue-600 hover:text-blue-500 hover:underline">Hai dimenticato la password?</button>
                            </div>
                            <div className="relative">
                                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        ...
                        >
                            {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Accedi al Workspace <ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" /></>}
                        </button>

                        {/* Social Login Section (Placeholder) */}
                        <div className="relative">
                            <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-gray-300"></div></div>
                            <div className="relative flex justify-center text-sm"><span className="px-2 bg-white text-gray-500">Oppure continua con</span></div>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                            <button type="button" onClick={handleFakeLogin} className="w-full inline-flex justify-center py-2.5 px-4 border border-gray-300 rounded-lg shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 transition-colors">
                        ...
                        <div className="text-center mt-4">
                            <p className="text-sm text-gray-600">Non hai ancora un account? <button type="button" onClick={() => setIsRegisterOpen(true)} className="font-semibold text-blue-600 hover:text-blue-700 hover:underline">Registrati ora</button></p>
                        </div>
                    </form>
                    <RegisterDialog isOpen={isRegisterOpen} onClose={() => setIsRegisterOpen(false)} onRegisterSuccess={() => { setError(null); alert("Registrazione avvenuta! Controlla l'email."); }} />
