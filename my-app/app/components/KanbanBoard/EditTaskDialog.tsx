...
export default function EditTaskDialog({
    ...
    // --- STATI NUOVI (Commenti e Allegati) ---
    const [comments, setComments] = useState<Comment[]>([]);
    const [newComment, setNewComment] = useState('');
    const [attachments, setAttachments] = useState<Attachment[]>([]);
    const fileInputRef = useRef<HTMLInputElement>(null);

    // Sincronizzazione dati iniziali
    useEffect(() => {
        if (isOpen && task) {
            setTitle(task.title);
            ...
            // MOCK: Carichiamo dati finti se non esistono nel task reale
            // In produzione, questi dati arriverebbero dal backend
            setComments([
            ...
            if (task.attachments > 0) {
                setAttachments([
                    { id: 'f1', name: 'Documentazione.pdf', size: '2.4 MB', type: 'pdf' }
                ]);
            } else {
                setAttachments([]);
            }
    ...
    const handleSave = (e: React.FormEvent) => {
        e.preventDefault();
        const newCategories = boardCategories.filter(c => selectedCategoryIds.includes(c.id.toString()));
        ...
        const updatedTask: Task = {
            ...
            comments: comments.length,       // Aggiorna contatore
            attachments: attachments.length  // Aggiorna contatore
        };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            {/* Dialog Container - Aumentata larghezza a max-w-5xl per layout a 2 colonne */}
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-5xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col h-[90vh]">
                ...
                {/* Body: Layout Grid a 2 Colonne */}
                <div className="flex-1 overflow-hidden flex flex-col lg:flex-row">

                    {/* COLONNA SINISTRA: Contenuto Principale (Scrollabile) */}
                    <div className="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar border-b lg:border-b-0 lg:border-r border-slate-100">
                        <form id="edit-task-form" onSubmit={handleSave} className="space-y-8">
                    ...
                    {/* COLONNA DESTRA: Commenti / Attività */}
                    <div className="w-full lg:w-1/3 bg-slate-50 flex flex-col h-full">
                ...
                {/* Footer Actions (Sottile barra inferiore) */}
                <div className="px-6 py-4 border-t border-slate-100 bg-white flex justify-end gap-3 z-10">
                    ...
                    <button
                        type="submit"
                        form="edit-task-form"
                        className="px-6 py-2 bg-slate-900 text-white text-sm font-semibold rounded-lg hover:bg-slate-800 transition-all flex items-center gap-2 shadow-lg shadow-slate-900/10"
                    >
